...
for (const x of DATA) {
	const [policy, letterUnparsed, password] = x.split(' ')
  const [policyMin, policyMax] = policy.split('-').map(Number)
  const [letter, unused] = letterUnparsed.split(':') 
  
  
  const found = password.split(letter).length - 1;
  
  // Part One
	if (_.inRange(found, policyMin, policyMax + 1)) {
    validPasswordsPartOne++
  }
  
  // Part Two
  const firstLetter = password.charAt(policyMin - 1)
  const lastLetter = password.charAt(policyMax - 1)
  
  if ((firstLetter == letter && lastLetter != letter) || (firstLetter != letter && lastLetter == letter)) {
